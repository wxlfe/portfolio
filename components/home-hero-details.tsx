...
function detectBrowser(): string | undefined {
  ...
  if (hasBrand("microsoft edge") || /EdgA?|EdgiOS\//.test(ua))
    return `${devicePrefix}Edge`;
  if (hasBrand("opera") || /OPR\/|OPiOS\//.test(ua))
    return `${devicePrefix}Opera`;
  if (hasBrand("firefox") || /Firefox\/|FxiOS\//.test(ua))
    return `${devicePrefix}Firefox`;
  if (hasBrand("samsung internet") || /SamsungBrowser\//.test(ua))
    return `${devicePrefix}Samsung Internet`;
  if (hasBrand("chrome") || /Chrome\/|CriOS\//.test(ua))
    return `${devicePrefix}Chrome`;
  if (
    hasBrand("safari") ||
    (/Safari\//.test(ua) &&
      !/Chrome\/|CriOS\/|EdgA?|EdgiOS\/|OPR\/|OPiOS\//.test(ua))
  ) {
 ...
 */
async function resolveLocationFromClient(
...
export function HomeHeroDetails() {
        ...
        <>
          ...
          {typeof data.isReturningVisitor === "boolean" ? (
            <span className="user_data">
              {data.isReturningVisitor ? "returning" : "first-time"}
            </span>
          ) : null}{" "}
          visitor
          {data.location ? (
            <>
              {" "}
              near <span className="user_data">{data.location}</span>
            </>
          ) : null}
          .
        </>
  ...
  const browserDeviceOsLine = (
    <>
      {data.browser && data.deviceType && data.os ? (
        <>
          You&apos;re using a <span className="user_data">{data.browser}</span>{" "}
          browser on a <span className="user_data">{data.deviceType}</span>{" "}
          running <span className="user_data">{data.os}</span>.
        </>
      ) : null}
      {data.browser && data.deviceType && !data.os ? (
        <>
      ...
      {data.browser && !data.deviceType && data.os ? (
        <>
          You&apos;re using a <span className="user_data">{data.browser}</span>{" "}
          browser running <span className="user_data">{data.os}</span>.
        </>
      ) : null}
      {!data.browser && data.deviceType && data.os ? (
        <>
          You&apos;re using a{" "}
          <span className="user_data">{data.deviceType}</span> running{" "}
  ...
  if (data.loadMs) {
    detailLines.push({
      ...
      content: (
        <>
          This page loaded for you in{" "}
          <span className="user_data">{data.loadMs}ms</span>
          {data.connectionType ? (
            <>
              {" "}
              over a <span className="user_data">
                {data.connectionType}
              </span>{" "}
              connection.
            </>
